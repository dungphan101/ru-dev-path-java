...
public class SiteGeoDaoRedisImpl implements SiteGeoDao {
    ...
    public Set<Site> findAll() {
        try (Jedis jedis = jedisPool.getResource()) {
            ...
            List<Response<Map<String, String>>> resPipelines = new ArrayList<>(keys.size());

            Pipeline pipelined = jedis.pipelined();
            for (String key : keys) {
                Response<Map<String, String>> site = pipelined.hgetAll(key);
                resPipelines.add(site);
            }
            pipelined.sync();

            resPipelines.forEach(resSite -> {
                Map<String, String> siteMap = resSite.get();
                if (!siteMap.isEmpty()) {
